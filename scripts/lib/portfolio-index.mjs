...
export async function collectPortfolioIssues({
  ...
  checkExactKeys(manifest, ['schemaVersion', 'owner', 'profileRepository', 'source', 'projects'], 'manifest', issue);
  ...
  if (typeof manifest.profileRepository !== 'string'
      || !manifest.profileRepository.startsWith(`${manifest.owner}/`)) {
    issue('repository-owner', `manifest.profileRepository must be owned by ${manifest.owner}`);
  }
  ...
  if (!projectNames.has(manifest.profileRepository)) {
    issue('repository-owner', 'manifest.profileRepository must be a selected project');
  }
...
export function renderProjectsMarkdown(manifest, snapshot) {
  ...
  const rows = selectedRepositories(manifest, snapshot).map(({ repository }) => {
    ...
    const pushed = repository.fullName === manifest.profileRepository
      ? 'self-indexed'
      : repository.pushedAt.slice(0, 10);
...
export async function fetchGitHubSnapshot(manifest, { fetchImpl = fetch, retrievedAt = new Date().toISOString() } = {}) {
  const selected = new Set(manifest.projects.map(({ repository }) => repository));
  const found = new Map();
  let endpoint = `${manifest.source.apiBaseUrl}/users/${encodeURIComponent(manifest.owner)}/repos?type=owner&sort=full_name&direction=asc&per_page=100`;
  let pageCount = 0;
  while (endpoint && found.size < selected.size) {
    pageCount += 1;
    if (pageCount > 10) throw new Error(`GitHub metadata pagination exceeded 10 pages for ${manifest.owner}`);
    let response;
    try {
      ...
      throw new Error(`GitHub metadata request failed for ${manifest.owner}: ${error.message}`);
    }
    if (!response.ok) {
      throw new Error(`GitHub metadata request failed for ${manifest.owner}: HTTP ${response.status}`);
    }
    const values = await response.json();
    if (!Array.isArray(values)) throw new Error(`GitHub metadata response for ${manifest.owner} is not a repository list`);
    for (const value of values) {
      if (selected.has(value.full_name)) found.set(value.full_name, normalizeApiRepository(value));
    }
    endpoint = nextPageUrl(response);
  }

  const missing = [...selected].filter((repository) => !found.has(repository));
  if (missing.length > 0) throw new Error(`GitHub metadata is missing selected repositories: ${missing.join(', ')}`);
  const repositories = manifest.projects.map(({ repository }) => found.get(repository));
...
export function diffRepositorySnapshots(manifest, expected, actual) {
  const issues = [];
  ...
  for (const repository of expected.repositories) {
    ...
    const comparedKeys = repository.fullName === manifest.profileRepository
      ? snapshotRepositoryKeys.filter((key) => !['updatedAt', 'pushedAt'].includes(key))
      : snapshotRepositoryKeys;
